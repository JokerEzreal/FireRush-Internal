...
namespace mono {
    ...
    // VTable / static fields
    mono_class_vtable_fn                 class_vtable           = nullptr;
    mono_field_static_get_value_fn       field_static_get_value = nullptr;

    // Type reflection
    mono_class_get_type_fn               class_get_type         = nullptr;
    mono_type_get_object_fn              type_get_object        = nullptr;

    // Array
    mono_array_length_fn                 array_length           = nullptr;
    ...
    bool initialize() {
        // firerush is an older Unity 4.x game -- it uses mono.dll (not mono-2.0-bdwgc.dll)
        s_mono_module = GetModuleHandleA("mono.dll");
        if (!s_mono_module) {
            // Fallback: try the newer variant just in case
        ...
        resolve(s_mono_module, "mono_class_vtable",                class_vtable);
        resolve(s_mono_module, "mono_field_static_get_value",      field_static_get_value);

        resolve(s_mono_module, "mono_class_get_type",              class_get_type);
        resolve(s_mono_module, "mono_type_get_object",             type_get_object);

        resolve(s_mono_module, "mono_array_length",                array_length);
