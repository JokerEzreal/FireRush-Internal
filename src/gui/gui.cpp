...
float horizontal_slider(const unity::Rect& pos, float value, float left, float right) {
    MonoMethod* method = unity::methods().GUI_HorizontalSlider_4;
    if (!method) return value;
...
int screen_width() {
    ...
    if (!method) return 1400;  // firerush default
...
int screen_height() {
    ...
    if (!method) return 900;  // firerush default
